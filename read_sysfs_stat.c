...
ssize_t rawdata_to_stat_dot1(const struct sysfs_stat stat_info, const int rawdata_fd, const int stat_fd) {
    ...
    if(num_read_bytes > 0) {
        ...
        if(percentage_int == 100)
            ...
            buff_len = snprintf(stat_buff, 256, "%2d.%s", percentage_int, rawdata_buff2);
        write(stat_fd, WHITESPACE, stat_info.column_width - buff_len);
        write(stat_fd, stat_buff, buff_len);
    }
...
ssize_t rawdata_to_stat_dot3(const struct sysfs_stat stat_info, const int rawdata_fd, const int stat_fd) {
    ...
    if(num_read_bytes > 0) {
        rawdata_buff[num_read_bytes] = '\0';
        strcpy(rawdata_buff2, &rawdata_buff[num_read_bytes-3]);
        rawdata_buff[num_read_bytes-3] = '\0';
...
ssize_t rawdata_to_stat_dot4(const struct sysfs_stat stat_info, const int rawdata_fd, const int stat_fd) {
    ...
    if(num_read_bytes > 0) {
        rawdata_buff[num_read_bytes] = '\0';
        strcpy(rawdata_buff2, &rawdata_buff[num_read_bytes-4]);
        rawdata_buff[num_read_bytes-4] = '\0';
