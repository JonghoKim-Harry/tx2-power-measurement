...
static void cnngov_b_init(void *data);
static int32_t cnngov_b_get_target_freq();
static void cnngov_b_print_gpugov(int fd);

// Interface
struct gpugov cnngov_b = {

    .name = "gpugov-cnngov_b",
    .init = cnngov_b_init,
    .get_target_freq = cnngov_b_get_target_freq,
    .print_gpugov = cnngov_b_print_gpugov
};
...
static int32_t cnngov_b_get_target_freq() {
    ...
    if(gpuutil < gpu_down_threshold)
        ...
        if(emcutil > emc_down_threshold)
            next_freq = scale_down_by_n_of_max(freq, scale_down_factor);
        else if(emcutil < emc_up_threshold)
            next_freq = gpugov_info.max_gpufreq;
        else
            next_freq = freq;
    }
    else
...
static void cnngov_b_print_gpugov(int fd) {
    ...
    buff_len = snprintf(buff, MAX_BUFFLEN, "\n - EMC DOWN THRESHOLD: %d.%d", (emc_down_threshold / 10000), (emc_down_threshold % 10000));
    write(fd, buff, buff_len);
