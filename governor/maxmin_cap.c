...
static void maxmin_cap_init(void *data);
static int32_t maxmin_cap_get_target_freq();
static void maxmin_cap_print_gpugov(int fd);

// Interface
struct gpugov maxmin_cap = {

    .name = "gpugov-maxmin_cap",
    .init = maxmin_cap_init,
    .get_target_freq = maxmin_cap_get_target_freq,
    .print_gpugov = maxmin_cap_print_gpugov
};
...
static int emc_cap_threshold         = 200000;   // 20% (unit: x0.0001%)
static int gpu_cap_threshold         = 500;      // 50% (unit: x0.0001%)
...
static int32_t cap_max_freq(int32_t next_freq, int64_t emcutil, int16_t gpuutil) {

    int32_t capped_max_freq;

    // Decide capped maximum frequency
    if(emcutil > emc_cap_threshold)
...
static int32_t maxmin_cap_get_target_freq() {
    ...
    if(gpuutil > gpu_up_threshold) {
        ...
        next_freq = cap_max_freq(next_freq, emcutil, gpuutil);
    }
...
static void maxmin_cap_print_gpugov(int fd) {
    ...
    buff_len = snprintf(buff, MAX_BUFFLEN, "\n - EMC CAP THRESHOLD: %d.%d", (emc_cap_threshold / 10000), (emc_cap_threshold % 10000));
    write(fd, buff, buff_len);
